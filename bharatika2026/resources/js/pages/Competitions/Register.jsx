  ...
  return (
    <div style={{ marginBottom: '24px' }}>
      ...
      <div
        ...
        style={{ width: '100%', padding: '20px', background: '#0a0a05', border: `1.5px dashed ${error ? '#FF5555' : (fileName ? C.gold : '#444')}`, borderRadius: '8px', textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s ease' }}
      >
...
function SectionCard({ title, children, accent = C.gold, delay = "0s" }) {
  return (
    <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, padding: '2.25rem', marginBottom: '2rem', borderRadius: '4px', boxShadow: '0 10px 40px rgba(0,0,0,0.5)', animation: `contentFadeIn 0.8s cubic-bezier(0.22, 1, 0.36, 1) ${delay} both` }}>
  ...
  return (
    <MainLayout>
      ...
      <div style={{ background: '#0F0A05', minHeight: 'calc(100vh - 52px)', paddingTop: 52, position: 'relative' }}>
        ...
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: '60px 1.5rem 6rem', position: 'relative', zIndex: 10 }}>
          <header style={{ textAlign: 'center', marginBottom: '3.5rem', animation: 'headerFadeIn 1s cubic-bezier(0.22, 1, 0.36, 1) both' }}>
            ...
            <h1 style={{ fontFamily: "'Cinzel', serif", fontSize: 'clamp(24px, 4vw, 42px)', fontWeight: 700, color: C.cream, margin: '0 0 0.75rem', lineHeight: 1.3, letterSpacing: '2px', textTransform: 'uppercase' }}>
          ...
          <form onSubmit={submit}>
                ...
                {memberCount > 0 ? Array(memberCount).fill(null).map((_, i) => (
                  <div key={i} style={{ marginBottom: '2.5rem', padding: '1.5rem', border: '1px solid #333', background: 'rgba(0,0,0,0.3)' }}>
                    ...
                    <div style={{ marginBottom: '1.5rem' }}>
                      ...
                      <input type="text" value={data.members[i]?.name || ''} onChange={e => updateMember(i, 'name', e.target.value)} placeholder="Masukkan nama lengkap..." style={{ width: '100%', padding: '12px', boxSizing: 'border-box', background: '#0F0A05', border: '1px solid #333', color: '#fff', fontFamily: "'EB Garamond', serif", fontSize: 16, outline: 'none' }} />
                    </div>
            ...
            <button type="submit" disabled={processing} style={{ width: '100%', padding: '20px', border: 'none', background: processing ? '#333' : C.gold, color: '#000', fontFamily: "'Cinzel', serif", fontSize: '13px', fontWeight: 800, letterSpacing: '4px', cursor: 'pointer', textTransform: 'uppercase', transition: 'all 0.2s ease' }}>
