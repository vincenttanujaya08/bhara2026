...
// ─── Changes: Added ConfirmModal for password update & logout ───────
...
const GLOBAL_STYLES = `
    ...
    .pf-tabs { display: flex; border-bottom: 1px solid rgba(200,168,75,0.15); margin-top: clamp(1.25rem,2.5vw,1.75rem); overflow-x: auto; -webkit-overflow-scrolling: touch; }
    .pf-edit-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem; }
    .pf-tab { padding: clamp(8px,1.2vw,10px) clamp(14px,2vw,22px); font-family: 'Cinzel', serif; font-size: clamp(9px,1vw,10px); font-weight: 600; letter-spacing: 2.5px; text-transform: uppercase; cursor: pointer; border: none; border-bottom: 2px solid transparent; background: transparent; color: #E8D9A0; opacity: 0.4; white-space: nowrap; flex-shrink: 0; transition: opacity 0.18s, color 0.18s, border-color 0.18s, transform 0.1s; outline: none; user-select: none; }
    ...
    .pf-btn { display: inline-flex; align-items: center; justify-content: center; padding: clamp(10px,1.5vw,13px) clamp(18px,2.5vw,28px); font-family: 'Cinzel', serif; font-size: clamp(9px,1vw,10px); font-weight: 700; letter-spacing: 3px; text-transform: uppercase; cursor: pointer; border-radius: 4px; text-decoration: none; transition: opacity 0.15s, transform 0.12s, background 0.15s, border-color 0.15s, box-shadow 0.15s; outline: none; user-select: none; white-space: nowrap; }
    .pf-btn:disabled { cursor: not-allowed; opacity: 0.4 !important; transform: none !important; pointer-events: none; }
    .pf-btn-outline { background: transparent; border: 1px solid rgba(200,168,75,0.3); color: #E8D9A0; opacity: 0.65; }
    .pf-btn-outline:hover { opacity: 1; border-color: #C8A84B; background: rgba(200,168,75,0.05); }
    .pf-btn-outline:active { opacity: 1; transform: scale(0.96); }
    .pf-btn-gold { background: linear-gradient(135deg, #C8A84B, #B8941E); border: none; color: #0F0A05; box-shadow: 0 2px 8px rgba(200,168,75,0.15); }
    .pf-btn-gold:hover { opacity: 0.88; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(200,168,75,0.25); }
    .pf-btn-gold:active { transform: scale(0.96); box-shadow: none; }
    .pf-btn-red { background: rgba(139,26,26,0.45); border: 1px solid rgba(224,128,128,0.3); color: #E08080; opacity: 0.85; }
    .pf-btn-red:hover { opacity: 1; background: rgba(139,26,26,0.75); border-color: #E08080; }
    .pf-btn-red:active { transform: scale(0.96); }
    input:-webkit-autofill, input:-webkit-autofill:hover, input:-webkit-autofill:focus { -webkit-text-fill-color: #E8D9A0; -webkit-box-shadow: 0 0 0px 1000px transparent inset; transition: background-color 5000s ease-in-out 0s; }
    @media (max-width: 600px) { .pf-edit-grid { grid-template-columns: 1fr; } .pf-hero { align-items: center; justify-content: center; text-align: center; } }
    @media (max-width: 480px) { .pf-stat-pills { justify-content: center; } .pf-stat-pills > * { flex: 1; min-width: 70px; } }
`
...
function StatPill({ label, value, color }) {
    return (
        <div style={{ textAlign: 'center', background: 'rgba(200,168,75,0.05)', border: '1px solid rgba(200,168,75,0.12)', padding: 'clamp(7px,1.2vw,10px) clamp(12px,2vw,18px)' }}>
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 'clamp(18px,3vw,24px)', color, margin: 0, fontWeight: 700, lineHeight: 1, letterSpacing: 1 }}>{value}</p>
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 8, color, opacity: 0.6, margin: '5px 0 0', letterSpacing: 2, textTransform: 'uppercase', fontWeight: 600 }}>{label}</p>
        </div>
    )
}
...
function SectionHeader({ label, accentColor = C.gold }) {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.25rem' }}>
            ...
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: accentColor, letterSpacing: 3, textTransform: 'uppercase', margin: 0, fontWeight: 700 }}>{label}</p>
        </div>
    )
}
...
function FlashMessage({ type, message }) {
    ...
    return (
        <div style={{ background: `${color}10`, border: `1px solid ${color}40`, borderLeft: `3px solid ${color}`, padding: '0.85rem 1.25rem', marginTop: '1.25rem' }}>
            <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 16, fontWeight: 600, color, margin: 0 }}>{message}</p>
        </div>
    )
}
...
function InfoRow({ label, value, mono = false }) {
    return (
        <div style={{ display: 'flex', gap: '1rem', padding: '0.65rem 0', borderBottom: '1px solid rgba(200,168,75,0.07)', alignItems: 'baseline', flexWrap: 'wrap' }}>
            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: C.gold, opacity: 0.6, letterSpacing: 2, textTransform: 'uppercase', margin: 0, minWidth: 90, flexShrink: 0, fontWeight: 600 }}>{label}</p>
            <p style={{ fontFamily: mono ? "'Cinzel', serif" : "'EB Garamond', serif", fontSize: mono ? 15 : 18, color: C.cream, margin: 0, opacity: value ? 0.9 : 0.3, letterSpacing: mono ? 1 : 0, wordBreak: 'break-word', fontWeight: mono ? 600 : 400 }}>{value || '—'}</p>
        </div>
    )
}
...
function EditField({ label, name, value, onChange, type = 'text', placeholder, error }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <label style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: focused ? C.gold : 'rgba(200,168,75,0.55)', letterSpacing: 2, textTransform: 'uppercase', fontWeight: 600, transition: 'color 0.2s' }}>{label}</label>
            <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder}
                onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
                style={{ background: 'rgba(200,168,75,0.04)', border: `1.5px solid ${focused ? C.gold : error ? C.red : 'rgba(200,168,75,0.2)'}`, borderRadius: 10, padding: 'clamp(9px,1.2vw,11px) 14px', color: C.cream, fontFamily: "'EB Garamond', serif", fontSize: 17, outline: 'none', transition: 'border-color 0.2s', caretColor: C.gold, width: '100%', boxSizing: 'border-box' }}
            />
...
export default function Profile({ user, stats = {}, flash = {} }) {
    ...
    const [modal, setModal] = useState({
        open:    false,
        type:    null,   // 'password' | 'profile' | 'logout'
        loading: false,
    })
    ...
    // ── Open modal helpers ──────────────────────────────────────────
    const openModal = (type) => setModal({ open: true, type, loading: false })
    ...
    const handleConfirm = () => {
        ...
        if (modal.type === 'password') {
            ...
            passForm.put('/profile/password', {
                preserveScroll: true,
                onSuccess: () => { passForm.reset(); closeModal() },
                onError:   () => setModal(m => ({ ...m, loading: false })),
            })
        }
    ...
    const MODAL_CONFIG = {
        profile: {
            ...
            description:  'Data diri Anda akan diperbarui sesuai isian yang baru saja diubah.',
            detail:       `Nama: ${editForm.data.name} · Instansi: ${editForm.data.instansi}`,
        ...
        password: {
            ...
            description:  'Password akun Anda akan diubah sekarang juga. Pastikan Anda mengingat password baru.',
            detail:       'Setelah diganti, gunakan password baru untuk login berikutnya.',
            confirmLabel: 'Ya, Ganti Password',
            variant:      'danger',
            icon:         'shield',
        },
        logout: {
            title:        'Keluar dari Bharatika',
                ...
                <div className="pf-wrap">
                    ...
                    <div className="pf-hero">
                        ...
                        <div style={{ flex: 1, minWidth: 180 }}>
                            <p style={{ fontFamily: "'Cinzel', serif", fontSize: 10, letterSpacing: 4, color: C.gold, opacity: 0.6, textTransform: 'uppercase', margin: '0 0 0.35rem', fontWeight: 700 }}>Peserta Bharatika 2026</p>
                            ...
                            <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 'clamp(13px,1.5vw,16px)', color: C.cream, opacity: 0.5, margin: 0, wordBreak: 'break-word' }}>{user?.email}</p>
                        </div>
                    ...
                    <div className="pf-tabs">
                        {TABS.map(t => (
                            <button key={t.key} className={`pf-tab${activeTab === t.key ? ' active' : ''}`} onClick={() => setActiveTab(t.key)}>{t.label}</button>
                        ))}
                    </div>
                        ...
                        <div style={{ marginTop: '1.5rem' }}>
                            <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                ...
                                <div style={{ padding: 'clamp(8px,1.2vw,10px) 14px', background: 'rgba(200,168,75,0.02)', border: '1px solid rgba(200,168,75,0.1)', borderRadius: 10 }}>
                                    <p style={{ fontFamily: "'Cinzel', serif", fontSize: 9, color: C.gold, opacity: 0.45, letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 4px', fontWeight: 700 }}>Email — tidak dapat diubah</p>
                                ...
                                <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', flexWrap: 'wrap', paddingTop: '0.5rem' }}>
                                    ...
                                    {/* ── Triggers profile confirm modal ── */}
                                    <Btn variant="gold" onClick={() => openModal('profile')} disabled={editForm.processing}>
                                        Simpan Perubahan
                                    </Btn>
                                </div>
                            </div>
                        ...
                        <div style={{ marginTop: '1.5rem' }}>
                            <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                ...
                                <div style={{ padding: 'clamp(8px,1.2vw,10px) 14px', background: 'rgba(200,168,75,0.03)', border: '1px solid rgba(200,168,75,0.08)', borderRadius: 8 }}>
                                    <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 14, color: C.cream, opacity: 0.45, margin: 0, lineHeight: 1.6 }}>Password minimal 6 karakter.</p>
                                </div>
                                <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                                    <Btn onClick={() => passForm.reset()}>Reset</Btn>
                                    {/* ── Triggers password confirm modal ── */}
                                    <Btn variant="gold" onClick={() => openModal('password')} disabled={passForm.processing}>
                                        Update Password
                                    </Btn>
                                </div>
                            </div>
                    ...
                    {activeTab === 'danger' && (
                        <div style={{ marginTop: '1.5rem' }}>
                            <div style={cardStyle}>
                                ...
                                {/* ── Triggers logout confirm modal ── */}
                                <Btn onClick={() => openModal('logout')}>
